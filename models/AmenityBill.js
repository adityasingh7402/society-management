...
AmenityBillSchema.pre('save', function(next) {
  this.remainingAmount = this.totalAmount - this.paidAmount;
  next();
});

// Method to generate bill number
AmenityBillSchema.methods.generateBillNumber = async function(session) {
  const date = new Date();
  const prefix = 'AMN/' + date.getFullYear().toString().substr(-2) + 
...
AmenityBillSchema.methods.calculateLateFee = function() {
