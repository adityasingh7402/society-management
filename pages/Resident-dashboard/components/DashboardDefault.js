...
const AndroidDashboard = ({ onLoaded }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-50 to-gray-100 pb-20 relative isolate">
      ...
      <div className="relative m-1 rounded-3xl h-52 overflow-hidden shadow-lg">
        {/* Image Slider */}
        <div 
          ref={sliderRef}
      ...
      <div className="fixed bottom-24 right-6 z-[45] pointer-events-auto drop-shadow-2xl">
        ...
        <button
          ...
          className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transform transition-all duration-300 active:scale-95 ${
            fabOpen 
      ...
      {categoryPopup && (
        <div className={`fixed inset-0 bg-black/50 z-50 flex items-end transition-all duration-300 ${
          ...
          <div className={`bg-white w-full rounded-t-3xl max-h-[80vh] overflow-y-auto transition-transform duration-300 ease-out shadow-2xl ${
              ...
              <div className="grid grid-cols-2 gap-3">
                {categoryPopup.items.map((item, index) => (
                  <Link key={index} href={item.href} passHref>
                    <div
                      className={`flex flex-col items-center p-5 bg-white/80 backdrop-blur-sm rounded-xl hover:bg-white/90 transition-all duration-200 active:scale-95 border border-gray-100 shadow-lg ${
