...
const ProductChat = () => {
  ...
  // Separate effect for fetching messages after both resident and product data are loaded
  useEffect(() => {
    if (residentData && productData && buyerId) {
      fetchMessages();
    }
  }, [residentData, productData, buyerId]);
  ...
  const fetchProductData = async () => {
    try {
      ...
      // Only proceed if we have both resident data and product data
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const token = localStorage.getItem('Resident');
      const receiverId = residentData._id === productData.sellerId ? buyerId : productData.sellerId;
      
      const response = await axios.post('/api/Product-Api/send-message', {
        productId,
        receiverId,
        message: newMessage,
      }, {
  ...
  if (loading || !residentData || !productData || loadingMessages) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        ...
        >
          {!residentData ? 'Loading your profile...' : 
           !productData ? 'Loading product details...' : 
           'Loading messages...'}
        </motion.p>
        <motion.p
          initial={{ y: 10, opacity: 0 }}
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => router.push('/Resident-dashboard/components/Marketplace')}
      ...
      <AnimatePresence>
        {productData && (
          ...
          >
            <Link
              href={`/Resident-dashboard/components/ProductDetail?id=${productId}`}
        ...
        <AnimatePresence mode="wait">
          {loadingMessages ? (
            ...
            >
              ...
              <p className="text-gray-500 mt-1">Start the conversation about this product! 🛒</p>
            </motion.div>
          ) : (
