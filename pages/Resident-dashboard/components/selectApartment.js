...
export default function FlatSelection({ residentId }) {
    ...
    useEffect(() => {
        ...
        const fetchApartmentStructure = async () => {
            setLoading(true);
            try {
                const response = await fetch(`/api/Society-Api/get-apartment-structure?societyId=${residentDetails.societyCode}`);
                if (!response.ok) {
                    ...
                    {selectedFloor !== "" && (
                        <div>
                            ...
                            >
                                ...
                                {blocks.find(b => b.blockName === selectedBlock)?.floors[selectedFloor]?.flats.map((flat) => (
