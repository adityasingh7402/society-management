    ...
    const handleUpload = async () => {
        if (croppedImage && securityId) {
            try {
                ...
                formData.append('securityId', securityId); // Append the security ID

                // Send the request to the backend API
                const response = await fetch('/api/Security-Api/update-security-image', {
                    method: 'POST',
                    body: formData,
                });
            ...
            alert('Please provide a security ID and crop the image.');
        }
    };
        ...
        <>
            ...
            <div className="min-h-screen bg-gray-100 flex flex-col space-y-10 items-center justify-center p-4">
                <div className="image-container flex justify-center items-center border border-gray-600 w-52 h-52 rounded-full">
                    <img className='rounded-full' src={securityDetails.userImage || "/profile.png"} alt="Profile" />
                </div>
