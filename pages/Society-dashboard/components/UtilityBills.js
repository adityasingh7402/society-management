  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (residentsResponse.ok) {
        ...
        setResidentList(residentsData);
        organizeResidentsByStructure(residentsData);
      }
      ...
      if (billsResponse.ok) {
        ...
        const summary = {
          ...
          totalAmount: billsData.bills.reduce((sum, bill) => sum +
            bill.baseAmount +
            (bill.additionalCharges?.reduce((acc, charge) => acc + charge.amount, 0) || 0) +
            (bill.penaltyAmount || 0), 0),
          totalPaidAmount: billsData.bills.filter(bill => bill.status === 'Paid')
            .reduce((sum, bill) => sum +
              bill.baseAmount +
              (bill.additionalCharges?.reduce((acc, charge) => acc + charge.amount, 0) || 0) +
              (bill.penaltyAmount || 0), 0),
          totalDueAmount: billsData.bills.filter(bill => bill.status !== 'Paid')
            .reduce((sum, bill) => sum +
              bill.baseAmount +
              (bill.additionalCharges?.reduce((acc, charge) => acc + charge.amount, 0) || 0) +
              (bill.penaltyAmount || 0), 0),
          totalPenalty: billsData.bills.reduce((sum, bill) => sum + (bill.penaltyAmount || 0), 0)
        };
        setSummaryData(summary);
        setBillHistory(billsData.bills);
        setFilteredHistory(billsData.bills);
      }
    } catch (error) {
    ...
    <div className="container mx-auto p-4">
      {loading ? (
        ...
        <div className="min-h-screen bg-gray-100">
          {actionLoading && <PreloaderSociety />}
          {/* Header */}
