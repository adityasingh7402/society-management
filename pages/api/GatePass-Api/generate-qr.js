...
export default async function handler(req, res) {
  ...
  try {
    const { qrData, societyName, flatNumber, guestName, validDates, vehicleDetails } = req.body;
    ...
    ctx.fillText(societyName, 600, 1100);

    ctx.font = '36px Arial';
    ctx.fillText(`Flat: ${flatNumber}`, 600, 1160);
    ctx.fillText(`Guest: ${guestName}`, 600, 1220);
    ctx.fillText(`Valid: ${validDates}`, 600, 1280);

    // Add vehicle details if present
    if (vehicleDetails) {
      ctx.fillText(
        `Vehicle: ${vehicleDetails.vehicleType} - ${vehicleDetails.registrationNumber}`, 
        600, 
        1340
      );
    }
